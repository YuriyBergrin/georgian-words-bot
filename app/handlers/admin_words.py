...
from app.handlers.common_helpers import CANCEL_TEXT, cancel_menu, reply_admin_menu
...
async def add_word_handler(message: Message, state: FSMContext, services: AppServices) -> None:
...
async def import_words_handler(message: Message, state: FSMContext, services: AppServices) -> None:
    if not await services.access.is_admin_user(message.from_user.id):
        await state.clear()
        await message.answer("Эта функция доступна только администратору", reply_markup=get_main_menu(False))
        return
...
async def export_words_handler(message: Message, state: FSMContext, services: AppServices) -> None:
    if not await services.access.is_admin_user(message.from_user.id):
        await state.clear()
        await message.answer("Эта функция доступна только администратору", reply_markup=get_main_menu(False))
        return
    rows = await services.admin_words.export_words_rows()
...
async def edit_word_handler(message: Message, state: FSMContext, services: AppServices) -> None:
    if not await services.access.is_admin_user(message.from_user.id):
        await state.clear()
        await message.answer("Эта функция доступна только администратору", reply_markup=get_main_menu(False))
        return
...
async def search_words_handler(message: Message, state: FSMContext, services: AppServices) -> None:
    if not await services.access.is_admin_user(message.from_user.id):
        await state.clear()
        await message.answer("Эта функция доступна только администратору", reply_markup=get_main_menu(False))
        return
...
async def cancel_admin_forms_handler(message: Message, state: FSMContext) -> None:
    await state.clear()
    await reply_admin_menu(message)
